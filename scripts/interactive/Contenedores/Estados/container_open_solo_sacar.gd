## ContainerOpenSoloSacar
# Estado abierto del cofre con transferencia de SOLO SACAR.
#
# Lógica 2: el jugador SOLO PUEDE SACAR objetos del contenedor.
#   - Al abrir: todos los ítems del contenedor pasan al jugador (Contenedor → Player).
#   - El jugador NO puede meter objetos dentro del contenedor.
#   - Cuando el contenedor queda vacío, regresa automáticamente a CLOSED.
...
# Al entrar al estado se abre el cofre y se transfiere el contenido al jugador.
func start() -> void:
...
# Si el jugador vuelve a presionar interact, intenta sacar lo que quede.
...
# Transfiere todos los objetos del contenedor al jugador (Contenedor → Player).
func _transfer_container_to_player() -> void:
	...
	if container_items.is_empty():
		print("[SoloSacar] El contenedor está vacío, no hay nada que sacar")
		return
	...
	if transfer_done:
		...
		print("[SoloSacar] Objetos del contenedor transferidos al jugador")
	else:
		print("[SoloSacar] No se pudo transferir ningún objeto al jugador")
...
func _get_container_inventory() -> Node:
	...
	if inventory == null:
		push_warning("ContainerOpenSoloSacar: nodo de inventario no encontrado -> " + str(inventory_node_path))
		return null
	return inventory
...
func _get_player_inventory() -> Node:
	...
	if player_node == null:
		push_warning("ContainerOpenSoloSacar: jugador interactuando no encontrado")
		return null
	var inventory := player_node.get_node_or_null(player_inventory_path)
	if inventory == null:
		push_warning("ContainerOpenSoloSacar: PlayerInventory no encontrado -> " + str(player_inventory_path))
		return null
	return inventory
...
func _bind_interaction_area_signals() -> void:
	...
	if _interaction_area == null:
		push_warning("ContainerOpenSoloSacar: Area2D no encontrado en " + str(interaction_area_path))
		return
