## ContainerOpenBidireccional
# Estado abierto del cofre con transferencia BIDIRECCIONAL.
#
# Lógica 1: el jugador puede SACAR y METER objetos al contenedor.
#
# Reglas de transferencia al abrir:
...
#   - Cualquier otro caso → no se transfiere nada.
# El jugador también puede volver a presionar "interact" para repetir la transferencia.
...
func start() -> void:
	...
	if _player_in_range:
		var transfer_result := _transfer_items_bidireccional()
		_close_after_player_to_container(transfer_result)
...
# se ejecuta otra transferencia bidireccional.
...
# Transferencia bidireccional según el estado de cada inventario.
func _transfer_items_bidireccional() -> int:
	...
	if player_has_items and not container_has_items:
		...
		print("[Bidireccional] Objetos del jugador transferidos al contenedor")
		...
		if transfer_done:
			...
			print("[Bidireccional] Objetos del contenedor transferidos al jugador")
			return _TRANSFER_CONTAINER_TO_PLAYER

		return _TRANSFER_NONE

	else:
		print("[Bidireccional] Sin transferencia: ambos inventarios en el mismo estado")
		return _TRANSFER_NONE


# Tras transferir del player al contenedor, vuelve al estado cerrado.
func _close_after_player_to_container(transfer_result: int) -> void:
	if transfer_result != _TRANSFER_PLAYER_TO_CONTAINER:
		return
...
# Si el contenedor está vacío, vuelve a CLOSED.
func _close_if_empty() -> void:
	var container_inventory := _get_container_inventory()
	if container_inventory == null:
		return
	if not container_inventory.get_all_items().is_empty():
		return
	if state_machine != null:
		state_machine.change_to(closed_state_path)
...
func _get_container_inventory() -> Node:
	...
	if inventory == null:
		push_warning("ContainerOpenBidireccional: nodo de inventario no encontrado -> " + str(inventory_node_path))
		return null
	return inventory
...
func _get_player_inventory() -> Node:
	...
	if player_node == null:
		push_warning("ContainerOpenBidireccional: jugador interactuando no encontrado")
		return null
	var inventory := player_node.get_node_or_null(player_inventory_path)
	if inventory == null:
		push_warning("ContainerOpenBidireccional: PlayerInventory no encontrado -> " + str(player_inventory_path))
		return null
	return inventory
...
func _bind_interaction_area_signals() -> void:
	...
	if _interaction_area == null:
		push_warning("ContainerOpenBidireccional: Area2D no encontrado en " + str(interaction_area_path))
		return
